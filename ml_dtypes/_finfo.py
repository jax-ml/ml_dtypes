...
class finfo(np.finfo):  # pylint: disable=invalid-name,missing-class-docstring
  ...
  _finfo_cache[_bcomplex32_dtype] = _finfo_cache[_bfloat16_dtype]
  _finfo_cache[_complex32_dtype] = np.finfo(np.float16)
  _finfo_name_map = {t.name: t for t in _finfo_cache}
  if hasattr(np.finfo, "_finfo_cache"):
    np.finfo._finfo_cache.update(_finfo_cache)  # pylint: disable=protected-access
